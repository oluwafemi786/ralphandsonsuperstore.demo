...
// Initialize EmailJS
(function() {
  emailjs.init("BPG9epfBDWN97V5aE"); // Replace with your actual public key
})();
...
function initContactForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (name.length < 2) { Swal.fire('Oops', 'Please enter your name.', 'warning'); return; }
    if (!/^\S+@\S+\.\S+$/.test(email)) { Swal.fire('Oops', 'Please enter a valid email.', 'warning'); return; }
    if (phone.length < 6) { Swal.fire('Oops', 'Please enter a valid phone number.', 'warning'); return; }
    if (message.length < 6) { Swal.fire('Oops', 'Please enter a message.', 'warning'); return; }
    ...
    emailjs.send("Ralph_son", "ralph_sons", {
      from_name: name,
      from_email: email,
      phone: phone,
      message: message
    ...
    .then(function() {
      ...
      const phoneNumber = "2348125479211"; // Replace with your WhatsApp number
      const whatsappMessage = `Hello Ralph and Sons,%0AName: ${name}%0AEmail: ${email}%0APhone: ${phone}%0AMessage: ${message}`;
...
function searchProduct() {
  ...
  products.forEach(product => {
    ...
    // Show product if search matches name or description
...
>>>>>>> db47bfc6990f0170974803357cbdfd27761b5e7c
